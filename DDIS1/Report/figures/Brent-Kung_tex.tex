...
\begin{tikzpicture}
	...
	\draw (propgen) to[open] ++(4,0) node {$= Initialize\_PropGen$};
	
	\node[draw,rectangle,minimum height=0.5cm,minimum width=0.8cm,fill=gray](DotOperator)at(0,1.5){};	
	\draw (DotOperator) to[open] ++(4,0) node {$= DotOperatorInvertedIn$};
	
	\node [draw,circle](klingel) at (8,2.5){};
		\draw (klingel) to[open] ++(4,0) node {$= DotOperator$};
	
	\draw [draw,circle,fill=gray](klingel) at (8,1.5){};
	
	\draw[fill] (.4,.6) circle  [radius=.5pt]
	\node [right] (.4,.6) {$=$ DotOperatorInverted};
